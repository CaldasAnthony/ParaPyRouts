...
def diag(data_base) :
    ...
    long_lat[0,0:reso_long+1] = np.linspace(variables['longitude'][0],variables['longitude'][reso_long],reso_long+1,dtype=np.float64)*np.pi/180.
    long_lat[1,0:reso_lat+1] = np.linspace(variables['latitude'][0],variables['latitude'][reso_lat],reso_lat+1,dtype=np.float64)*np.pi/180.
...
def latlongalt(Rp,h,r,rho,r_step,z_level,delta,delta_step,reso_lat,alpha,alpha_o_ref,alpha_o_ref_0,alpha_step,reso_long,phi_obli,\
    ...
    if Obliquity == False :
        ...
        q, = np.where(long_ref == alpha_norm)
...
def qoblicator(theta_range,x,x_range,x_ref,theta_number,phi_obli,alpha_o,alpha_o_ref,alpha_o_ref_0,alpha_step,reso_long,inv,refrac,begin,long_ref) :
    ...
    if x_range != begin and x_range != begin+1 :
        ...
        q_o, = np.where(long_ref == alpha_norm)
        q_o = q_o[0]
    ...
    if x_range != begin and x_range != begin+1 :
        ...
        if phi_obli == np.pi/2. :
            ...
            if alpha_o%alpha_step < alpha_step/2. :
                ...
                alpha_norm = alpha_o - alpha_o%alpha_step + alpha_step
            q_o = np.where(long_ref == alpha_norm)[0]
