...
def Boxes_interlopation(P,T,h2o,Rp,g0,M_atm,number,P_comp,T_comp,Q_comp,species,x_species,M_species,c_species,ratio,dom_rank,\
...
def dx_correspondance(p_grid,q_grid,z_grid,data,x_step,r_step,theta_step,Rp,g0,h,t,reso_long,reso_lat,n_lay_rank,Middle=False,\
    ...
    dx_init = np.ones((r_size,theta_size,x_size),dtype = np.int)*(-1)
    order_init = np.ones((6,r_size,theta_size,x_size),dtype = np.int)*(-1)
    dx_init_opt = np.ones((r_size,theta_size,x_size),dtype = np.float64)*(-1)
    if Integral == True :
        pdx_init = np.ones((r_size,theta_size,x_size),dtype = np.float64)*(-1)
	...
	i = i_r
        ...
        for j in range(theta_size) :
                            ...
                            if dist < Lmax :
                                    ...
                                    if z_1_2 != -1 or z_1_3 != -1 :
                                        if z_1_2 != -1 :
                                            if z_1_3 != -1 :
                                                ...
                                                for i_z in range(3) :
                                                    ...
                                                    if np.str(integ[0]) == 'inf' :
                                                        pdx_init[i_r,j,x] += P_1/(R_gp*T_1)*N_A*(np.sqrt((Rp+z_2)**2-(Rp+r)**2) - np.sqrt((Rp+z_ref[i_z])**2-(Rp+r)**2))
                                                        print('We did a correction in the integration cell (%i,%i,%i), with %f' %(i,j,k,pdx_init[i_r,j,x])), 'initial result', integ[0]
                                                    else :
                                                        pdx_init[i_r,j,x] += integ[0]

                                                    z_2 = z_ref[i_z]

                                                    if Ord == True :
                                                        order_init[0,i_r,j,x] = z_grid[i,j,k-1+ind[0,i_z]]
                                                        order_init[1,i_r,j,x] = p_grid[i,j,k-1+ind[1,i_z]]
                                                        order_init[2,i_r,j,x] = q_grid[i,j,k-1+ind[2,i_z]]
                                                        order_init[3,i_r,j,x] = k-1+ind[0,i_z]
                                                        order_init[4,i_r,j,x] = k-1+ind[1,i_z]
                                                        order_init[5,i_r,j,x] = k-1+ind[2,i_z]
                                                ...
                                                for i_z in range(2) :
                                                    ...
                                                    if Ord == True :
                                                        order_init[0,i_r,j,x] = z_grid[i,j,k-1+ind[0,i_z]]
                                                        order_init[1,i_r,j,x] = p_grid[i,j,k-1+ind[1,i_z]]
                                                        order_init[2,i_r,j,x] = q_grid[i,j,k-1]
                                                        order_init[3,i_r,j,x] = k-1+ind[0,i_z]
                                                        order_init[4,i_r,j,x] = k-1+ind[1,i_z]
                                                        order_init[5,i_r,j,x] = k-1
                                            ...
                                            for i_z in range(2) :
                                                ...
                                                if Ord == True :
                                                    order_init[0,i_r,j,x] = z_grid[i,j,k-1+ind[0,i_z]]
                                                    order_init[1,i_r,j,x] = p_grid[i,j,k-1]
                                                    order_init[2,i_r,j,x] = q_grid[i,j,k-1+ind[1,i_z]]
                                                    order_init[3,i_r,j,x] = k-1+ind[0,i_z]
                                                    order_init[4,i_r,j,x] = k-1
                                                    order_init[5,i_r,j,x] = k-1+ind[1,i_z]
                                        ...
                                        if np.str(integ[0]) == 'inf' :
                                            pdx_init[i_r,j,x] = P_1/(R_gp*T_1)*N_A*(np.sqrt((Rp+z_2)**2-(Rp+r)**2) - np.sqrt((Rp+z_1)**2-(Rp+r)**2))
                                            print('We did a correction in the integration cell (%i,%i,%i), with %f' %(i,j,k,pdx_init[i_r,j,x])), 'initial result', integ[0]
                                        else :
                                            pdx_init[i_r,j,x] = integ[0]
                                        ...
                                        if Ord == True :
                                            order_init[0,i_r,j,x] = z_grid[i,j,k-1]
                                            order_init[1,i_r,j,x] = p_grid[i,j,k-1]
                                            order_init[2,i_r,j,x] = q_grid[i,j,k-1]
                                            order_init[3,i_r,j,x] = k-1
                                            order_init[4,i_r,j,x] = k-1
                                            order_init[5,i_r,j,x] = k-1
                                    ...
                                    if p_grid[i,j,k] != p_grid[i,j,k-1] :
                                        ...
                                        if q_grid[i,j,k] != q_grid[i,j,k-1] :
                                            ...
                                            for i_z in range(2) :
                                                ...
                                                if Ord == True :
                                                    order_init[0,i_r,j,x] = z_grid[i,j,k-1]
                                                    order_init[1,i_r,j,x] = p_grid[i,j,k-1+ind[0,i_z]]
                                                    order_init[2,i_r,j,x] = q_grid[i,j,k-1+ind[1,i_z]]
                                                    order_init[3,i_r,j,x] = k-1
                                                    order_init[4,i_r,j,x] = k-1+ind[0,i_z]
                                                    order_init[5,i_r,j,x] = k-1+ind[1,i_z]
                                        ...
                                        if np.str(integ[0]) == 'inf' :
                                            pdx_init[i_r,j,x] = P_1/(R_gp*T_1)*N_A*(np.sqrt((Rp+z_2)**2-(Rp+r)**2) - np.sqrt((Rp+z_1)**2-(Rp+r)**2))
                                            print('We did a correction in the integration cell (%i,%i,%i), with %f' %(i,j,k,pdx_init[i,j,x])), 'initial result', integ[0]
                                        else :
                                            pdx_init[i_r,j,x] = integ[0]
                                ...
                                if mid == 1 :
                                    ...
                                    if center == 1 or center == 2 :
                                        ...
                                        if Ord == True :
                                            order_init[0,i_r,j,x] = z_grid[i,j,k]
                                            order_init[1,i_r,j,x] = p_grid[i,j,k]
                                            order_init[2,i_r,j,x] = q_grid[i,j,k]
                                            order_init[3,i_r,j,x] = k
                                            order_init[4,i_r,j,x] = k
                                            order_init[5,i_r,j,x] = k
                                                ...
                                                if z_2_2 != -1 :
                                                    if z_2_3 != -1 :
                                                        ...
                                                        for i_z in range(3) :
                                                            ...
                                                            if np.str(integ[0]) == 'inf' :
                                                                pdx_init[i_r,j,x] += P_1/(R_gp*T_1)*N_A*(np.sqrt((Rp+z_ref[i_z])**2-(Rp+r)**2) - np.sqrt((Rp+z_1)**2-(Rp+r)**2))
                                                                print('We did a correction in the integration cell (%i,%i,%i), with %f' %(i,j,k,pdx_init[i_r,j,x])), 'initial result', integ[0]
                                                            else :
                                                                pdx_init[i_r,j,x] += integ[0]
                            ...
                            if dist < Lmax :
                                ...
                                if z_grid[i,j,k] != z_grid[i,j,k-1] :
                                    ...
                                    if q_grid[i,j,k] != q_grid[i,j,k-1] and p_grid[i,j,k] != p_grid[i,j,k-1] :
                                        ...
                                        dx_init_opt[i_r,j,y] = L - x_ref[max]
                                        L -= dx_init_opt[i_r,j,y]
                                        order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,0]]
                                        order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[1,0]]
                                        order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[2,0]]
                                        order_init[3,i_r,j,y] = k-1+ind[0,0]
                                        order_init[4,i_r,j,y] = k-1+ind[1,0]
                                        order_init[5,i_r,j,y] = k-1+ind[2,0]
                                        y = y + 1

                                        dx_init_opt[i_r,j,y] = L - x_ref[mid]
                                        delta = L - x_ref[mid]
                                        L -= delta
                                        order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,1]]
                                        order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[1,1]]
                                        order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[2,1]]
                                        order_init[3,i_r,j,y] = k-1+ind[0,1]
                                        order_init[4,i_r,j,y] = k-1+ind[1,1]
                                        order_init[5,i_r,j,y] = k-1+ind[2,1]
                                        y = y + 1

                                        dx_init_opt[i_r,j,y] = L - x_ref[min]
                                        delta = L - x_ref[min]
                                        L -= delta
                                        order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,2]]
                                        order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[1,2]]
                                        order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[2,2]]
                                        order_init[3,i_r,j,y] = k-1+ind[0,2]
                                        order_init[4,i_r,j,y] = k-1+ind[1,2]
                                        order_init[5,i_r,j,y] = k-1+ind[2,2]
                                        ...
                                        if q_grid[i,j,k] != q_grid[i,j,k-1] :
                                            ...
                                            L -= dx_init_opt[i_r,j,y]
                                            order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,0]]
                                            order_init[1,i_r,j,y] = p_grid[i,j,k-1]
                                            order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[1,0]]
                                            order_init[3,i_r,j,y] = k-1+ind[0,0]
                                            order_init[4,i_r,j,y] = k-1
                                            order_init[5,i_r,j,y] = k-1+ind[1,0]
                                            y = y + 1
                                            ...
                                            order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,1]]
                                            order_init[1,i_r,j,y] = p_grid[i,j,k-1]
                                            order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[1,1]]
                                            order_init[3,i_r,j,y] = k-1+ind[0,1]
                                            order_init[4,i_r,j,y] = k-1
                                            order_init[5,i_r,j,y] = k-1+ind[1,1]
                                            ...
                                            if p_grid[i,j,k] != p_grid[i,j,k-1] :
                                                ...
                                                order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,0]]
                                                order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[1,0]]
                                                order_init[2,i_r,j,y] = q_grid[i,j,k-1]
                                                order_init[3,i_r,j,y] = k-1+ind[0,0]
                                                order_init[4,i_r,j,y] = k-1+ind[1,0]
                                                order_init[5,i_r,j,y] = k-1
                                                y = y + 1
                                                ...
                                                dx_init_opt[i_r,j,y] = L - x_pre
                                                L -= dx_init_opt[i_r,j,y]
                                                order_init[0,i_r,j,y] = z_grid[i,j,k-1]
                                                order_init[1,i_r,j,y] = p_grid[i,j,k-1]
                                                order_init[2,i_r,j,y] = q_grid[i,j,k-1]
                                                order_init[3,i_r,j,y] = k-1
                                                order_init[4,i_r,j,y] = k-1
                                                order_init[5,i_r,j,y] = k-1
                                    ...
                                    if q_grid[i,j,k] != q_grid[i,j,k-1] and p_grid[i,j,k] != p_grid[i,j,k-1] :
                                        ...
                                        L -= dx_init_opt[i_r,j,y]
                                        order_init[0,i_r,j,y] = z_grid[i,j,k-1]
                                        order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[0,0]]
                                        order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[1,0]]
                                        order_init[3,i_r,j,y] = k-1
                                        order_init[4,i_r,j,y] = k-1+ind[0,0]
                                        order_init[5,i_r,j,y] = k-1+ind[1,0]
                                        ...
                                        L -= delta
                                        order_init[0,i_r,j,y] = z_grid[i,j,k-1]
                                        order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[0,1]]
                                        order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[1,1]]
                                        order_init[3,i_r,j,y] = k-1
                                        order_init[4,i_r,j,y] = k-1+ind[0,1]
                                        order_init[5,i_r,j,y] = k-1+ind[1,1]
                                ...
                                if mid_y == 2 :
                                    ...
                                    if p_grid[i,j,k] != p_grid[i,j,k-1] or q_grid[i,j,k] != q_grid[i,j,k-1] or z_grid[i,j,k] != z_grid[i,j,k-1] :
                                        ...
                                        dx_init_opt[i_r,j,y] = L - x_step/2.
                                        order_init[0,i_r,j,y] = z_grid[i,j,k-1]
                                        order_init[1,i_r,j,y] = p_grid[i,j,k-1]
                                        order_init[2,i_r,j,y] = q_grid[i,j,k-1]
                                        order_init[3,i_r,j,y] = k-1
                                        ...
                                        dx_init_opt[i_r,j,y] = x_step/2.
                                        dx_init[i_r,j,y] = 1
                                        order_init[0,i_r,j,y] = z_grid[i,j,k]
                                        order_init[1,i_r,j,y] = p_grid[i,j,k]
                                        order_init[2,i_r,j,y] = q_grid[i,j,k]
                                        order_init[3,i_r,j,y] = k
                                        order_init[4,i_r,j,y] = k
                                        order_init[5,i_r,j,y] = k
                                    ...
                                    if z_grid[i,j,k] != z_grid[i,j,k-1] and center_y != 2 :
                                        ...
                                        if q_grid[i,j,k] != q_grid[i,j,k-1] or p_grid[i,j,k] != p_grid[i,j,k-1] :
                                            ...
                                            if q_grid[i,j,k] != q_grid[i,j,k-1] and p_grid[i,j,k] != p_grid[i,j,k-1] :
                                                ...
                                                dx_init_opt[i_r,j,y] = x_ref[min] - ex
                                                ex = x_ref[min]
                                                order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,0]]
                                                order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[1,0]]
                                                order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[2,0]]
                                                ...
                                                dx_init_opt[i_r,j,y] += x_ref[mid] - ex
                                                ex = x_ref[mid]
                                                order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,1]]
                                                order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[1,1]]
                                                order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[2,1]]
                                                ...
                                                dx_init_opt[i_r,j,y] += x_ref[max] - ex
                                                ex = x_ref[max]
                                                order_init[0,i_r,j,y] = z_grid[i,j,k-1+ind[0,2]]
                                                order_init[1,i_r,j,y] = p_grid[i,j,k-1+ind[1,2]]
                                                order_init[2,i_r,j,y] = q_grid[i,j,k-1+ind[2,2]]
            ...
            len = np.where(order_init[0,i_r,j,:] != -1)[0].size
